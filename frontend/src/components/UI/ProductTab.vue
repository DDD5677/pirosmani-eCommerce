<template>
	<div class="product__tab">
		<ul class="nav">
			<li class="nav-item">
				<button class="nav-link" :class="{ 'active': about }" @click="aboutHandler">
					О товаре
				</button>
			</li>
			<li class="nav-item">
				<button class="nav-link" :class="{ 'active': service }" @click="serviceHandler">
					Параметры
				</button>
			</li>
			<li class="nav-item">
				<button class="nav-link" :class="{ 'active': reviews }" @click="reviewsHandler">
					Отзывы
				</button>
			</li>
		</ul>
		<div class="tab-content">
			<div v-if="about" class="tab-pane">
			...
			<div v-if="service" class="tab-pane fade product__param">
			...
			<div v-if="reviews" class="tab-pane__reviews">
				...
				<div v-if="!reviewLoading" class="swiper__hidden">
						...
						<swiper-slide v-for="review in reviewsShow" class="reviews__item">
							<div class="item__top">
								<div class="item__top-img">
									<img :src="review.user.image" alt="" class="top__img" />
								</div>
								<div class="top__info">
									<h4 class="name__autor">
										<span>{{ review.user.name }} {{ review.user.surname }}</span>
										<img src="@/assets/images/reviews__autor-icon.svg" alt="" />
									</h4>

									<product-rating :width="'10px'" :rating="review.rating"></product-rating>
									<a href="" class="top__info-link">{{ review.user.email }}</a>
								</div>
							</div>
					...
					<div ref="next" class="swiper-button-next swiper-button-next2">
						<span>Еще</span>
						<img src="@/assets/images/product__reviews-arrow.svg" alt="" />
					</div>
				</div>
				<green-btn @click.prevent="toggleModalHandler" class="green__btn">
					Добавить отзыв
				</green-btn>
			</div>
		</div>
